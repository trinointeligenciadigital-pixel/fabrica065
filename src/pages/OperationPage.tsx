...
function errorMessage(error: unknown) {
  ...
  if (text.includes("PERMISSION_DENIED")) return "Seu perfil não permite registrar produção nesta câmara.";
...
function ConnectedOperation({ chamberToken }: { chamberToken: string }) {
  ...
  async function enter() {
    ...
    try {
      const result = await login({
        chamberToken,
        collaboratorId: collaboratorId as Id<"collaborators">,
        pin,
      });
      if (!result.ok) {
  ...
  async function submit() {
    if (!current?.valid || !selectedProduct || !selectedVariant || !sessionToken || !online) return;
    ...
    try {
      ...
      setReceipt({ movementId: result.movementId, quantityBase: result.quantityBase, occurredAt: result.occurredAt });
      setStep("success");
    } catch (error) {
  ...
  return (
    <div className="operation-shell">
      ...
      <div className="camera-context">
        <div><span className="context-symbol"><Snowflake size={20} /></span><div><small>Câmara atual</small><strong>{current.chamberName} — Produção</strong></div></div>
      ...
      <main className="operation-main">
        {step !== "success" && (
          <div className="step-progress" aria-label="Progresso do lançamento">
            ...
            <span className={step !== "confirm" ? "is-current" : "is-complete"}>2. Produção</span><i />
            <span className={step === "confirm" ? "is-current" : ""}>3. Confirmar</span>
          </div>
        )}

        {!current.canProduce ? (
          <section className="operation-panel access-error-panel"><span className="access-symbol"><UserRound size={25} /></span><h1>Produção não liberada</h1><p>Seu acesso permite outras operações, mas não o registro de produção nesta câmara.</p></section>
        ) : step === "product" ? (
          <section className="operation-panel">
            <div className="operation-title"><span className="operation-title-icon"><PackagePlus size={24} /></span><div><p>Produção</p><h1>O que entrou na câmara?</h1><span>Escolha o produto produzido.</span></div></div>
            {current.products.length === 0 ? <div className="empty-state compact-empty"><strong>Nenhum produto ativo</strong><p>Peça ao administrador para concluir os cadastros.</p></div> : (
              <div className="product-options">
                {current.products.map((product) => {
                  const hasVariant = product.kind === "saborizado"
                    ? current.flavors.length > 0
                    : current.formats.some((item) => item.productId === product.id);
                  return (
                    <button key={product.id} disabled={!hasVariant} onClick={() => selectProduct(product.id)}>
                      <span><strong>{product.name}</strong><small>{product.kind === "saborizado" ? "Escolher sabor" : hasVariant ? "Escolher formato" : "Sem formato ativo"}</small></span><ChevronRight size={21} />
                    </button>
                  );
                })}
              </div>
          ...
          <section className="operation-panel">
            ...
            <div className="operation-title compact"><div><p>{selectedProduct.kind === "saborizado" ? "Sabor" : "Formato"}</p><h1>{selectedProduct.kind === "saborizado" ? "Qual sabor foi produzido?" : "Qual pacote foi produzido?"}</h1><span>{selectedProduct.name}</span></div></div>
            <div className="product-options">
              {variants.map((variant) => <button key={variant.id} onClick={() => selectVariant(variant.id)}><span><strong>{variant.name}</strong><small>{variant.gramsPerPackage ? formatBaseQuantity(variant.gramsPerPackage, "grama") + " por pacote" : "Gelo saborizado"}</small></span><ChevronRight size={21} /></button>)}
            </div>
          </section>
          ...
          <section className="operation-panel">
            ...
            <label className="quantity-field"><span>Quantos pacotes entraram?</span><div><input inputMode="numeric" pattern="[0-9]*" autoFocus value={quantity} onChange={(event) => setQuantity(event.target.value.replace(/\D/g, ""))} placeholder="0" aria-describedby="quantity-help" /><b>pacotes</b></div><small id="quantity-help">{selectedVariant.gramsPerPackage ? "Total calculado: " + formatBaseQuantity(totalBase, "grama") : "Informe apenas pacotes completos."}</small></label>
            <button className="button button-primary operation-submit" disabled={!quantity || Number(quantity) <= 0} onClick={() => setStep("confirm")}>Revisar lançamento <ChevronRight size={20} /></button>
          </section>
        ) : step === "confirm" && selectedProduct && selectedVariant ? (
          <section className="operation-panel">
            <button className="back-button" onClick={() => setStep("quantity")}><ArrowLeft size={18} />Corrigir quantidade</button>
            <div className="operation-title compact"><div><p>Confirmação</p><h1>Confira antes de registrar</h1><span>A entrada será gravada no histórico imutável.</span></div></div>
            <dl className="confirmation-list"><div><dt>Câmara</dt><dd>{current.chamberName}</dd></div><div><dt>Movimentação</dt><dd>Entrada de produção</dd></div><div><dt>Produto</dt><dd>{selectedProduct.name} · {selectedVariant.name}</dd></div><div className="confirmation-total"><dt>Quantidade</dt><dd>{quantity} pacotes{selectedVariant.gramsPerPackage ? " · " + formatBaseQuantity(totalBase, "grama") : ""}</dd></div><div><dt>Responsável</dt><dd>{current.collaboratorName}</dd></div></dl>
            ...
            <button className="button button-primary operation-submit" disabled={!online || saving} onClick={() => void submit()}><Check size={20} />{saving ? "Registrando…" : "Confirmar entrada"}</button>
          </section>
        ) : step === "success" && selectedProduct && selectedVariant && receipt ? (
          <section className="operation-panel success-panel">
            <span className="success-symbol"><CircleCheck size={38} /></span><p>Entrada registrada</p><h1>{quantity} pacotes adicionados</h1><span>{selectedProduct.name} · {selectedVariant.name}</span>
            <div className="success-note"><strong>Saldo atualizado em tempo real.</strong><span>Registro {String(receipt.movementId).slice(-8)} · {new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit", timeZone: "America/Cuiaba" }).format(receipt.occurredAt)}</span></div>
            <button className="button button-primary operation-submit" onClick={reset}>Registrar outra produção</button>
          </section>
        ) : null}
      </main>
...
function IntegrationRequired() {
  return (
    <div className="operation-shell">
      <header className="operation-header"><Link to="/" className="operation-brand"><span className="brand-mark"><Snowflake size={21} /></span><span><strong>Estoque</strong><b>065</b></span></Link></header>
      <main className="operation-main"><section className="operation-panel access-error-panel"><span className="access-symbol"><WifiOff size={25} /></span><h1>Ambiente não configurado</h1><p>Clerk e Convex precisam estar configurados para registrar operações reais.</p></section></main>
    </div>
  );
}
